...
//  ToDoList
//
//  Created by Mahmoud Alaa on 24/11/2025.
//
...
struct AnimatedSideBar<Content: View, MenuView: View, Background: View>: View {
    ...
    // MARK: - State & Content Builders
    @Binding var showMenu: Bool
    
    // Content takes UIEdgeInsets to adjust its internal layout based on the safe area
    ...
    var body: some View {
        GeometryReader {
            ...
            // Complex safe area calculation from the image.
            // In modern SwiftUI, you typically use `proxy.safeAreaInsets`,
            // but we'll include the explicit calculation shown in the screenshot for accuracy.
            ...
            HStack(spacing: 0) {
                ...
                .mask { RoundedRectangle(cornerRadius: progress * cornerRadius) }
                .scaleEffect(rotatesWhenExpands ? 1 - (progress * 0.1) : 1, anchor: .trailing)
                .rotation3DEffect(.init(degrees: rotatesWhenExpands ? (progress * -15) : 0),
    ...
    var dragGesture:  some Gesture  {
        DragGesture()
            .updating($isDragging) { _, out, _ in
                ...
                let translation = isDragging ? max(min(value.translation.width + lastOffsetX, sideMenuWidth), 0) : 0
                offsetX = translation
                calculateProgress()
            }.onEnded{ value in
                guard value.startLocation.x > 10 else { return }
